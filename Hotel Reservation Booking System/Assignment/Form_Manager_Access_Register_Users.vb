...
Public Class Form_Manager_Access_Register_Users
        ...
        Try
            ...
            Else
                ...
                If dr.Read() And dr(4) = "Manager" Then ' If the record can be queried, it means passing verification, but the information must same as DataRow 4 (Staff Role Rows) which show 'Manager' Role in Database Table, then only allow to open Report form.   
                    MessageBox.Show("Access to Register Users sucessfully", "Access Successful", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Form_Register_Users.Show()
                    Me.Close()
                    Form_MainPage.Hide()
                Else
                    MessageBox.Show("Register Users is only for Manager, Staff don't have the permission to access", "Access Failure", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                End If
            End If
        Catch
    ...
    Private Sub cbShowPassword_CheckedChanged(sender As Object, e As EventArgs) Handles chkShowPassword.CheckedChanged
    ...
    Private Sub Form_Manager_Access_Register_Users_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        txtPassword.UseSystemPasswordChar = True 'Password Text Will Show in Password Character while entering Manager Access (Register User) Form
